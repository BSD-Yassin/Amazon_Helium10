...
def login(email_credential,password_credential):
    ...
    WebDriverWait(driver, 10).until(EC.visibility_of_element_located((By.ID,"loginform-email")))
...
def config_set():
        ...
        elif not config.has_section("PREFS"):
...
def config_read():
    ...
    login = infos_section["email_login"]
    password = infos_section["password_login"]
